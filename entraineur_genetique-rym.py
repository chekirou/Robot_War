...
def setupArena():
    ...
    for i in range(0,5):
        addObstacle(row=1,col=i)

    for i in range(2,7):
        addObstacle(row=i,col=3)
...
def mute(individu,pMute):
    ...
    for e in individu:
        if random() < pMute:
            nouvelIndividu.append (e+ choice([-1,1])*(uniform(-1, 1)/10))
        else:
...
for iteration in range(nbruns):
    ...
    bestEvalIt = 0
    taillePop = 100
    maxEvaluations = 1000 # budget en terme de nombre de robots évalués au total
