...
# History:
#   2017-01-30 14h48
#   2017-02-28 clone from multiplot2.py
#   2019-04-09 11h46: convert to Python3 (Note: in Python 3, map returns an iterator, which you can only iterate over once)
...
if len(args.filenames) == 1:
    ...
    for l in lines[0]:
        ...
        if i % args.resolution == 0:
            xData.append( l2[args.xIndex] )
            yData.append( l2[args.yIndex] )
            print ( str(l2[args.xIndex]) + "," + str(l2[args.yIndex]) )
        i = i + 1
    ...
    for i in range(0,len(linesList[0]),args.resolution):
        print ("i = ",i)
        xData.append( int(linesList[0][i][args.xIndex]) )
        l = []
        for j in range(len(linesList)):
            l.append( linesList[j][i][args.yIndex] )
        yData.append(l)
